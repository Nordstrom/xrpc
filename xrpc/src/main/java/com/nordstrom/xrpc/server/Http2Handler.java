...
public final class Http2Handler extends Http2ConnectionHandler implements Http2FrameListener {
  ...
  public int onDataRead(
    ...
    if (endOfStream) {
      // TODO(jkinkead): Will this trigger on a not found with a request writeBody?
      XrpcRequest request = ctx.channel().attr(XrpcConstants.XRPC_REQUEST).get();
      request.writeBody(data);
      HandlerAdapter handler = ctx.channel().attr(XrpcConstants.XRPC_HANDLER).get();
      try {
