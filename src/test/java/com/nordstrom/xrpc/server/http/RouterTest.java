...
class RouterTest {
  ...
  void addRoute() {
    ...
    AtomicReference<ImmutableSortedMap<Route, Handler>> routes = r.getRoutes();

    for (Route route : routes.get().descendingKeySet()) {
      Optional<Map<String, String>> groups = Optional.ofNullable(route.groups("/foo"));
      if (groups.isPresent()) {
        assertEquals(h1, routes.get().descendingMap().get(route));
      }
    }
    ...
    for (Route route : routes.get().descendingKeySet()) {
      ...
      if (groups.isPresent()) {
        assertEquals(h2, routes.get().descendingMap().get(route));
      }
    }
    ...
    for (Route route : routes.get().descendingKeySet()) {
      ...
      if (groups.isPresent()) {
        assertEquals(h3, routes.get().descendingMap().get(route));
      }
    }
    ...
    for (Route route : routes.get().descendingKeySet()) {
      ...
      if (groups.isPresent()) {
        assertEquals(h4, routes.get().descendingMap().get(route));
      }
    }
    ...
    for (Route route : routes.get().descendingKeySet()) {
      ...
      if (groups.isPresent()) {
        assertEquals(h5, routes.get().descendingMap().get(route));
      }
    }
